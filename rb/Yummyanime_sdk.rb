...
require_relative 'Yummyanime_types'
...
class YummyanimeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = YummyanimeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = YummyanimeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, YummyanimeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    YummyanimeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = YummyanimeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = YummyanimeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = YummyanimeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => YummyanimeError.new(
        "#{op}_allow",
        "YummyanimeSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = YummyanimeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = YummyanimeError.new(
        "graphql_error", "YummyanimeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
