...
The Ruby SDK for the Yummyanime API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "Yummyanime_sdk"
client = YummyanimeSDK.new(options)
```
...
raise a `YummyanimeError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Yummyanime_sdk.rb       -- Main SDK module
