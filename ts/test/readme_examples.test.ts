...
//     (\`new YummyanimeSDK(...)\` / \`YummyanimeSDK.test(...)\` become
//     \`YummyanimeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new YummyanimeSDK(...)\` and any
// \`YummyanimeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+YummyanimeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/YummyanimeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
