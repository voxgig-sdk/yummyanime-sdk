# Yummyanime SDK test runner

from __future__ import annotations
import os
import json

from yummyanime_sdk.utility.voxgig_struct import voxgig_struct as vs


class YummyanimeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            YummyanimeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = YummyanimeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = YummyanimeTestRunner.getenv("YUMMYANIME_TEST_LIVE")
        override = YummyanimeTestRunner.getenv("YUMMYANIME_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = YummyanimeTestRunner.getenv(key)
        ...
        explain = YummyanimeTestRunner.getenv("YUMMYANIME_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["YUMMYANIME_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if YummyanimeTestRunner._test_control is not None:
            return YummyanimeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                YummyanimeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            YummyanimeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = YummyanimeTestRunner.load_test_control()
...
def load_env_local():
    YummyanimeTestRunner.load_env_local()


def env_override(m):
    return YummyanimeTestRunner.env_override(m)


def entity_data(v):
    return YummyanimeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return YummyanimeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return YummyanimeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return YummyanimeTestRunner.load_test_control()


def live_client_options():
    return YummyanimeTestRunner.live_client_options()


def live_delay_ms():
    return YummyanimeTestRunner.live_delay_ms()
