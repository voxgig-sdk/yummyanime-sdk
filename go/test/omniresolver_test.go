...
import (
	...
	omni "github.com/voxgig-sdk/yummyanime-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YummyanimeError); is {
				out := map[string]any{
					"name":    "YummyanimeError",
...
func omniCtx(arg any, client *sdk.YummyanimeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YummyanimeSDK); is {
			client = live
