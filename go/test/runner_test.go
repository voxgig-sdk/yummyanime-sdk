...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("YUMMYANIME_TEST_LIVE") == "TRUE" ||
		os.Getenv("YUMMYANIME_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("YUMMYANIME_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.YummyanimeSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.YummyanimeError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.YummyanimeSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.YummyanimeError{Msg: msg, Code: code}
}
