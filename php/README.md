...
The PHP SDK for the Yummyanime API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Anime()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
print_r(array_map(fn($item) => $item->data_get(), $anime));
```
...
```php
require_once 'yummyanime_sdk.php';
$client = new YummyanimeSDK($options);
```
...
```
php/
├── yummyanime_sdk.php          -- Main SDK class
...
The main class (`yummyanime_sdk.php`) exports the SDK class
