...
class YummyanimeRetryFeature extends YummyanimeBaseFeature
{
    ...
    public function init(YummyanimeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (YummyanimeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(YummyanimeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(YummyanimeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
