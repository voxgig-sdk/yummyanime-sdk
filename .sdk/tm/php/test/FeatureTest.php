...
class FtCtrl extends YummyanimeControl
{
...
class FtClient
{
    ...
    private YummyanimeUtility $utility;

    public function __construct(YummyanimeUtility $utility, array $options)
    {
    ...
    public function get_utility(): YummyanimeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YummyanimeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public YummyanimeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YummyanimeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YummyanimeConfig::make_feature($name);
    ...
    private static function buildUrl(YummyanimeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YummyanimeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YummyanimeResponse($fetched);
        }
    ...
    private function fail(YummyanimeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YummyanimeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
