...
class YummyanimeMakeError
{
    public static function call(?YummyanimeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new YummyanimeContext([], null);
        }
        $op = $ctx->op ?? new YummyanimeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new YummyanimeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof YummyanimeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new YummyanimeError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof YummyanimeError) {
