...
class YummyanimeMakeRequest
{
    public static function call(YummyanimeContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new YummyanimeResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new YummyanimeResponse($fetched);
        } else {
